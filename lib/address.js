...
const STREET_SUFFIX_TOKENS = new Set([
  ...
  'ter','terrace','trl','trail','pt','point','manor'
]);

// Spelled-out US state names → USPS abbreviations. Normalized in
// parseFreetextAddress so "South carolina 29150" parses as state="SC".
...
function expandStateNames(text) {
  let out = text;
  // Replace longest names first to avoid "south carolina" -> "SC carolina" via "south".
  const keys = Object.keys(STATE_NAME_MAP).sort((a, b) => b.length - a.length);
  for (const name of keys) {
    const re = new RegExp(`\\b${name}\\b`, 'gi');
    out = out.replace(re, STATE_NAME_MAP[name]);
  }
  return out;
}
...
function parseFreetextAddress(text) {
  ...
  // Strip trailing ", USA" / " USA" / " USA." then expand spelled-out states.
  const raw = expandStateNames(
    text.trim().replace(/,?\s+USA\.?\s*$/i, '').trim()
  );
